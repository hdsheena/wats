<?php
	...
	$TITLE = "Batch Add to Room"; 
	$BREADCRUMBS = array(array('name' => "Home", 'link'=>$CONFIG['webroot']),
						 array('name'=>"Batch Add to Room"),
						 array('name'=>"Step 1 - Choose Room"));
	...
	if ($_REQUEST['roomID'])
	{
		...
		{			
			...
			$res2 = assignDeviceToRoom($_REQUEST['deviceID'], $_REQUEST['roomID']);
	...
	else
	{
		...
		$roomform[] = array("Room:", "<select name='roomID'  id='focusme'>$roomopts</select>");
		$roomform[] = array("", "<input type='submit' name='submit' value='Select'>");
		
		$roomform = form('form', 'POST', '', Table::quick($roomform));
		
		print mainContentBox("Select Room", NULL, $roomform);
	}
?>
