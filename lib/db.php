...
print queryBox($query);
	...
	{
		$buildingName = makeSafe($buildingName);

		$query = "INSERT INTO `building` VALUES (NULL, '$buildingName');";

		return db_query($query);
	}

	function updateBuilding($buildingID, $buildingName)
	{
		$buildingID = makeSafe($buildingID);
		$buildingName = makeSafe($buildingName);

		$query = "UPDATE `building` SET buildingName='$buildingName' WHERE buildingID = '$buildingID';";
